...
ID3DX10Mesh*      CDXUTDirectionWidget::s_pD3D10Mesh = NULL;
...
HRESULT CDXUTDirectionWidget::StaticOnD3D10CreateDevice( ID3D10Device* pd3dDevice )
{
	...
	V_RETURN(DXUTCreateCylinder(pd3dDevice, 0.0f, 0.5f, 2.0f, 8, 2, &s_pD3D10Mesh));
...
HRESULT CDXUTDirectionWidget::OnRender10( D3DXCOLOR color, const D3DXMATRIX* pmView, const D3DXMATRIX* pmProj, const D3DXVECTOR3* pEyePt )
{
    ...
    s_pRenderTech->GetDesc(&techDesc);
	for (UINT iPass = 0; iPass < techDesc.Passes; ++iPass)
	{
		s_pRenderTech->GetPassByIndex(iPass)->Apply(0);

		V(s_pD3D10Mesh->DrawSubset(0));
	}
