...
public:
    ...
    void Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog = true, LPCTSTR pszControlTextureFilename = "UI\\DXUTControls.dds" );
    void Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog, LPCTSTR szControlTextureResourceName, HMODULE hControlTextureResourceModule );
    ...
    void SetCaptionText( const char *pwszText ) { StringCchCopy( m_wszCaption, sizeof(m_wszCaption)/sizeof(m_wszCaption[0]), pwszText); }
    ...
    HRESULT          SetTexture( UINT index, LPCTSTR strResourceName, HMODULE hResourceModule );
...
private:
    ...
    void InitDefaultElements( LPCTSTR pszControlTextureFilename );
    void InitDefaultElements( LPCTSTR pszControlTextureResourceName, HMODULE hControlTextureResourceModule );
...
public:
    ...
    int AddTexture( LPCTSTR strResourceName, HMODULE hResourceModule );
...
public:
    ...
    // Windows message handler
    virtual bool MsgProc( UINT , WPARAM , LPARAM ) { return false; }

    virtual bool HandleKeyboard( UINT , WPARAM , LPARAM ) { return false; }
    virtual bool HandleMouse( UINT , POINT , WPARAM , LPARAM ) { return false; }
...
public:
    ...
    virtual BOOL ContainsPoint( POINT ) { return false; }

    HRESULT GetTextCopy( LPTSTR strDest, UINT bufferCount );
...
public:
    ...
    int  GetTextSize()  { return lstrlen( m_pwszBuffer ); }
    ...
    bool InsertChar( int nIndex, char  ); // Inserts the char at specified index. If nIndex == -1, insert to the end.
...
protected:
    ...
    static UINT WINAPI Dummy_GetReadingString( HIMC, UINT, LPTSTR, PINT, BOOL*, PUINT ) { return 0; }
    ...
    static UINT (WINAPI * _GetReadingString)( HIMC, UINT, LPTSTR, PINT, BOOL*, PUINT );
...
protected:
    ...
    void TruncateCompString( bool bUseBackSpace = true, int iNeTSTRLen = 0 );
...
protected:
    ...
    struct CCandList
    {
        char awszCandidate[MAX_CANDLIST][256];
    ...
    struct CInputLocale
    {
        ...
        char m_wszLangAbb[3];  // Language abbreviation
        char m_wszLang[64];    // Localized language name
    };
    ...
    static LPTSTR  s_wszCurrIndicator;    // Points to an indicator string that corresponds to current input locale
    static char   s_aszIndicator[5][3];  // String to draw to indicate current input locale
    ...
    static char   s_wszReadingString[32];// Used only with horizontal reading window (why?)
