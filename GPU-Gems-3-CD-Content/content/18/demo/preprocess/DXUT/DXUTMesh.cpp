...
HRESULT CDXUTMesh::Create( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR strFilename )
{
    ...
    char *pLastBSlash = strrchr( strPath, '\\' );
    if( pLastBSlash )
        *(pLastBSlash + 1) = '\0';
    else
        *strPath = '\0';
                           ...
                           LPD3DXFILEDATA pFileData )
{
    ...
    D3DXMATERIAL* d3dxMtrls = (D3DXMATERIAL*)pMtrlBuffer->GetBufferPointer();
    hr = CreateMaterials( "", pd3dDevice, d3dxMtrls, m_dwNumMaterials );
                           ...
                           D3DXMATERIAL* pd3dxMaterials, DWORD dwMaterials )
{
    ...
    HRESULT hr;
    hr = CreateMaterials( "", pd3dDevice, pd3dxMaterials, dwMaterials );
...
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::CreateMaterials( LPCTSTR strPath, IDirect3DDevice9 *pd3dDevice, D3DXMATERIAL* d3dxMtrls, DWORD dwNumMaterials )
{
    ...
    if( d3dxMtrls && m_dwNumMaterials > 0 )
    {
        ...
        for( DWORD i=0; i<m_dwNumMaterials; i++ )
        {
            ...
            if( d3dxMtrls[i].pTextureFilename )
            {
				...
				StringCchCopy(strTextureTemp, MAX_PATH, d3dxMtrls[i].pTextureFilename);
                             ...
                             CDXUTMeshFrame* pParentFrame )
{
    ...
    if( Guid == TID_D3DRMFrame )
    {
		...
		StringCchCopy(strName, 512, strAnsiName);
        strName[511] = 0;
...
HRESULT CDXUTMeshFile::CreateFromResource( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR strResource, LPCTSTR strType )
{
	...
	StringCchCopy(strTypeAnsi, MAX_PATH, strType);
    strTypeAnsi[MAX_PATH-1] = 0;

	StringCchCopy(strResourceAnsi, MAX_PATH, strResource);
...
HRESULT CDXUTMeshFile::Create( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR strFilename )
{
	...
	StringCchCopy(strPathANSI, MAX_PATH, strPath);
