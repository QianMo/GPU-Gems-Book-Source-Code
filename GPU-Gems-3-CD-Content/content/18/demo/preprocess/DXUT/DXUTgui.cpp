...
//--------------------------------------------------------------------------------------
void CDXUTDialog::Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog, LPCTSTR pszControlTextureFilename )
{
    m_pManager = pManager;
    if( bRegisterDialog )
        pManager->RegisterDialog( this );
    InitDefaultElements( pszControlTextureFilename ); // requires a CDXUTDialogResourceManager to store textures/fonts
}


//--------------------------------------------------------------------------------------
void CDXUTDialog::Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog, LPCTSTR szControlTextureResourceName, HMODULE hControlTextureResourceModule )
{
    m_pManager = pManager;
    if( bRegisterDialog )
        pManager->RegisterDialog( this );
    InitDefaultElements( szControlTextureResourceName, hControlTextureResourceModule ); // requires a CDXUTDialogResourceManager to store textures/fonts
}
...
//--------------------------------------------------------------------------------------
int CDXUTDialogResourceManager::AddTexture( LPCTSTR strResourceName, HMODULE hResourceModule )
{
    // See if this texture already exists
    for( int i=0; i < m_TextureCache.GetSize(); i++ )
    {
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::SetTexture( UINT index, LPCTSTR strResourceName, HMODULE hResourceModule )
{
    // If this assert triggers, you need to call CDXUTDialog::Init() first.  This change
    // was made so that the DXUT's GUI could become seperate and optional from DXUT's core.  The 
    // creation and interfacing with CDXUTDialogResourceManager is now the responsibility 
...
HRESULT CDXUTDialogResourceManager::CreateTexture( UINT iTexture )
{
    ...
    {
        LPCTSTR pID = pTextureNode->nResourceID ? (LPCTSTR)(size_t)pTextureNode->nResourceID : pTextureNode->strFilename;
        ...
        if( FAILED(hr) )
        {
            return DXTRACE_ERR( "D3DXCreateTextureFromResourceEx", hr );
        }
    }
    else
    {
...
//--------------------------------------------------------------------------------------
void CDXUTDialog::InitDefaultElements( LPCTSTR pszControlTextureFilename )
{
    SetTexture( 0, pszControlTextureFilename );

    InitDefaultElementsHelper();
}


//--------------------------------------------------------------------------------------
void CDXUTDialog::InitDefaultElements( LPCTSTR pszControlTextureResourcename, HMODULE hResourceModule )
{
    SetTexture( 0, pszControlTextureResourcename, hResourceModule );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTStatic::GetTextCopy( LPTSTR strDest, UINT bufferCount )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTEditBox::GetTextCopy( LPTSTR strDest, UINT bufferCount )
{
    assert( strDest );
...
void CDXUTEditBox::ParseFloatArray( float *pNumbers, int nCount )
{
    ...
    while( nWritten < nCount && *pToken != '\0' )
    {
        // Skip leading spaces
        while( *pToken == ' ' )
            ++pToken;
...
LONG (WINAPI * CDXUTIMEEditBox::_ImmGetCompositionString)( HIMC, DWORD, LPVOID, DWORD ) = CDXUTIMEEditBox::Dummy_ImmGetCompositionString;
DWORD (WINAPI * CDXUTIMEEditBox::_ImmGetCandidateList)( HIMC, DWORD, LPCANDIDATELIST, DWORD ) = CDXUTIMEEditBox::Dummy_ImmGetCandidateList;
...
UINT (WINAPI * CDXUTIMEEditBox::_GetReadingString)( HIMC, UINT, LPTSTR, PINT, BOOL*, PUINT ) = CDXUTIMEEditBox::Dummy_GetReadingString; // Traditional Chinese IME
...
char     CDXUTIMEEditBox::s_aszIndicator[5][3] = // String to draw to indicate current input locale
            {
                "En",
                "??",
                "??",
...
LPTSTR    CDXUTIMEEditBox::s_wszCurrIndicator = CDXUTIMEEditBox::s_aszIndicator[0];  // Points to an indicator string that corresponds to current input locale
...
char     CDXUTIMEEditBox::s_wszReadingString[32];
...
void CDXUTIMEEditBox::CheckInputLocale()
{
    ...
    {
        char wszLang[5] = "En";
        //GetLocaleInfo( MAKELCID( LOWORD( s_hklCurrent ), SORT_DEFAULT ), LOCALE_SABBREVLANGNAME, wszLang, 5 );
        s_wszCurrIndicator[0] = wszLang[0];
        s_wszCurrIndicator[1] = (CHAR)tolower( wszLang[1] );
    }
}
...
void CDXUTIMEEditBox::SetupImeApi()
{
    ...
    if ( !s_hDllIme )
        return;
    _GetReadingString = (UINT (WINAPI*)(HIMC, UINT, LPTSTR, PINT, BOOL*, PUINT))
...
void CDXUTIMEEditBox::TruncateCompString( bool bUseBackSpace, int iNeTSTRLen )
{
    if( !s_bInsertOnType )
        return;

    int cc = (int) strlen( s_CompString.GetBuffer() );
    assert( iNeTSTRLen == 0 || iNeTSTRLen >= cc );
    ...
    if( iNeTSTRLen < cc )
    {
        for( int i = 0; i < cc - iNeTSTRLen; ++i )
        {
            SendMessage( DXUTGetHWND(), WM_KEYDOWN, VK_BACK, 0 );  // Backspace character
            SendMessage( DXUTGetHWND(), WM_CHAR, VK_BACK, 0 );
        }
...
void CDXUTIMEEditBox::GetReadingWindowOrientation( DWORD dwId )
{
    ...
    if( !s_bHorizontalReading && ( dwId & 0x0000FFFF ) == LANG_CHT )
    {
        ...
        LONG lRc = RegOpenKeyEx( HKEY_CURRENT_USER, wszRegPath, 0, KEY_READ, &hKey );
...
void CDXUTIMEEditBox::GetPrivateReadingString()
{
    ...
    DWORD dwErr = 0;
    char *pwszReadingStringBuffer = NULL;  // Buffer for when the IME supports GetReadingString()
    char *TSTR = 0;
    ...
    if( _GetReadingString )
    {
        ...
        if( dwReadingStrLen )
        {
            TSTR = pwszReadingStringBuffer = (LPTSTR)HeapAlloc( GetProcessHeap(), 0, sizeof(char) * dwReadingStrLen );
            ...
            dwReadingStrLen = _GetReadingString( hImc, dwReadingStrLen, TSTR, (PINT)&dwErr, &bVertical, &uMaxUiLen );
        }
    ...
    else
    {
        ...
        switch( dwId )
        {
            ...
            case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
            {
                ...
                int nTcharSize = ( osi.dwPlatformId == VER_PLATFORM_WIN32_NT ) ? sizeof(char) : sizeof(char);
    ...
    if( bUnicodeIme )
    {
        ...
        for( i = 0; i < dwReadingStrLen; ++i ) // dwlen > 0, if known IME
        {
            ...
            s_CandList.awszCandidate[i][0] = TSTR[i];
    ...
    else
    {
        ...
        for( i = 0, j = 0; i < dwReadingStrLen; ++i, ++j ) // dwlen > 0, if known IME
        {
            ...
            if( GetLocaleInfo( MAKELCID( GetLanguage(), SORT_DEFAULT ),
                                LOCALE_IDEFAULTANSICODEPAGE,
                                wszCodePage,
                                sizeof(wszCodePage)/sizeof(wszCodePage[0]) ) )
            {
                uCodePage = strtoul( wszCodePage, NULL, 0 );
            }
			// XXX
            ...
            if( wParam )
            {
                ...
                if( phKL )
                {
                    ...
                    for( UINT i = 0; i < cKL; ++i )
                    {
                            ...
                            case LANG_CHINESE:
                                switch( SUBLANGID( LOWORD( phKL[i] ) ) )
                                {
                                    ...
                                    case SUBLANG_CHINESE_TRADITIONAL:
                                        ...
                                        break;
                                    default:    // unsupported sub-language
                                        //GetLocaleInfo( MAKELCID( LOWORD( phKL[i] ), SORT_DEFAULT ), LOCALE_SABBREVLANGNAME, wszDesc, 128 );
                                        Locale.m_wszLangAbb[0] = wszDesc[0];
                                        Locale.m_wszLangAbb[1] = (CHAR)tolower( wszDesc[1] );
                        ...
                        GetLocaleInfo( MAKELCID( LOWORD( phKL[i] ), SORT_DEFAULT ), LOCALE_SLANGUAGE, wszDesc, 128 );
            ...
            if( s_CandList.bShowWindow && PtInRect( &s_CandList.rcCandidate, pt ) )
            {
                ...
                } else
                {
                    ...
                    for( UINT i = 0; i < s_CandList.dwCount; ++i )
                    {
                        ...
                        {
                            // Haven't found it.
                            nEntryStart += lstrlen( s_CandList.awszCandidate[i] ) + 1;  // plus space separator
                        } else
                        {
    ...
    switch( uMsg )
    {
        ...
        case WM_IME_COMPOSITION:
            DXUTTRACE( "WM_IME_COMPOSITION\n" );
            {
                ...
                char wszCompStr[MAX_COMPSTRING_SIZE];
                ...
                *trapped = true;
                ...
                {
                    s_nCompCaret = _ImmGetCompositionString( hImc, GCS_CURSORPOS, NULL, 0 );
                    if( s_nCompCaret < 0 )
                ...
                if ( lParam & GCS_RESULTSTR )
                {
                    ...
                    lRet = _ImmGetCompositionString( hImc, GCS_RESULTSTR, wszCompStr, sizeof( wszCompStr ) );
                    if( lRet > 0 )
                    {
                        lRet /= sizeof(char);
                ...
                if ( lParam & GCS_COMPSTR )
                {
                    ...
                    lRet = _ImmGetCompositionString( hImc, GCS_COMPSTR, wszCompStr, sizeof( wszCompStr ) );
                    if( lRet > 0 )
                    {
                        lRet /= sizeof(char);  // Convert size in byte to size in char
                        wszCompStr[lRet] = 0;  // Force terminate
                        //
                        ...
                        if ( GetLanguage() == LANG_CHT && !GetImeId() )
                        {
                            if( lstrlen( s_CompString.GetBuffer() ) )
                            {
                                ...
                                ZeroMemory( (LPVOID)s_CompString.GetBuffer(), 4 * sizeof(char) );
                                s_bShowReadingWindow = true;
                ...
                {
                    lRet = _ImmGetCompositionString( hImc, GCS_COMPATTR, s_abCompStringAttr, sizeof( s_abCompStringAttr ) );
                    if( lRet > 0 )
                ...
                {
                    lRet = _ImmGetCompositionString(hImc, GCS_COMPCLAUSE, s_adwCompStringClause, sizeof( s_adwCompStringClause ) );
                    ...
                    *trapped = true;
                    ...
                    if( lpCandList )
                    {
                            ...
                            i++, j++ )
                        {
                            ...
                            LPTSTR pwsz = s_CandList.awszCandidate[j];
                            // For every candidate string entry,
                            // write [index] + Space + [String] if vertical,
                            // write [index] + [String] + Space if horizontal.
                            *pwsz++ = (char)( '0' + ( (j + 1) % 10 ) );  // Index displayed is 1 based
                            if( s_bVerticalCand )
                                *pwsz++ = ' ';
                            char *pwszNewCand = (LPTSTR)( (LPBYTE)lpCandList + lpCandList->dwOffset[i] );
                            while ( *pwszNewCand )
                                *pwsz++ = *pwszNewCand++;
                            if( !s_bVerticalCand )
                                *pwsz++ = ' ';
                        ...
                        if( !s_bVerticalCand )
                        {
                            ...
                            {
                                if( s_CandList.awszCandidate[i][0] == '\0' )
                                    break;
                            ...
                            wszCand[lstrlen(wszCand) - 1] = '\0';  // Remove the last space
        ...
        case WM_SYSKEYUP:
            ...
            {
                PostMessage( GetFocus(), WM_KEYUP, (WPARAM)VK_MENU, ( lAlt & 0x01ff0000 ) | 0xC0000001 );
            }   
            else if ( !( lCtrl & 0x80000000 ) && wParam != VK_CONTROL && ( GetAsyncKeyState( VK_CONTROL ) & 0x8000 ) == 0 )
            {
                PostMessage( GetFocus(), WM_KEYUP, (WPARAM)VK_CONTROL, ( lCtrl & 0x01ff0000 ) | 0xC0000001 );
            }
            else if ( !( lShift & 0x80000000 ) && wParam != VK_SHIFT && ( GetAsyncKeyState( VK_SHIFT ) & 0x8000 ) == 0 )
            {
                PostMessage( GetFocus(), WM_KEYUP, (WPARAM)VK_SHIFT, ( lShift & 0x01ff0000 ) | 0xC0000001 );
            }
...
void CDXUTIMEEditBox::RenderComposition( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    const char *pwszComp = s_CompString.GetBuffer();
...
void CDXUTIMEEditBox::RenderIndicator( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    char *pwszIndicator = s_bEnableImeSystem ? s_wszCurrIndicator : s_aszIndicator[0];
...
//--------------------------------------------------------------------------------------
bool CUniBuffer::InsertChar( int nIndex, char ch )
{
    assert( nIndex >= 0 );
...
bool CUniBuffer::RemoveChar( int nIndex )
{
    ...
    MoveMemory( m_pwszBuffer + nIndex, m_pwszBuffer + nIndex + 1, sizeof(char) * ( lstrlen( m_pwszBuffer ) - nIndex ) );
    m_bAnalyseRequired = true;
    return true;
}
...
bool CUniBuffer::InsertString( int nIndex, const char *pStr, int nCount )
{
    ...
    MoveMemory( m_pwszBuffer + nIndex + nCount, m_pwszBuffer + nIndex, sizeof(char) * ( lstrlen( m_pwszBuffer ) - nIndex + 1 ) );
...
HRESULT CUniBuffer::XtoCP( int nX, int *pCP, int *pnTrail )
{
    ...
    *pCP = 0; *pnTrail = FALSE;  // Default
    ...
    if( *pCP > lstrlen( m_pwszBuffer ) && *pnTrail == FALSE )
    {
...
void CDXUTIMEEditBox::Initialize()
{
    ...
    if( s_hDllImm32 )
    {
        ...
        GETPROCADDRESS( s_hDllImm32, ImmGetCompositionString, Temp );
        GETPROCADDRESS( s_hDllImm32, ImmGetCandidateList, Temp );
