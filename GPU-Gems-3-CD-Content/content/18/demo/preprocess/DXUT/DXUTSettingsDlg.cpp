...
//--------------------------------------------------------------------------------------
char* DXUTPresentIntervalToString( UINT pi );
char* DXUTMultisampleTypeToString(D3DMULTISAMPLE_TYPE MultiSampleType);
char* DXUTD3DDeviceTypeToString(D3DDEVTYPE devType);
char* DXUTVertexProcessingTypeToString(DWORD vpt);
...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init( CDXUTDialogResourceManager* pManager, LPCTSTR szControlTextureFileName )
{
    assert( pManager );
...
//--------------------------------------------------------------------------------------
void CD3DSettingsDlg::Init( CDXUTDialogResourceManager* pManager, LPCTSTR pszControlTextureResourcename, HMODULE hModule )
{
    assert( pManager );
...
{
    if( pd3dDevice == NULL )
        return DXUT_ERR_MSGBOX( "CD3DSettingsDlg::OnCreatedDevice", E_INVALIDARG );
...
HRESULT CD3DSettingsDlg::Refresh()
{
    ...
    if( NULL == pBestDeviceSettingsCombo )
        return DXUT_ERR_MSGBOX( "GetDeviceSettingsCombo", E_INVALIDARG );    
    ...
    if( pAdapterInfoList->GetSize() == 0 )
        return DXUT_ERR_MSGBOX( "CD3DSettingsDlg::OnCreatedDevice", DXUTERR_NOCOMPATIBLEDEVICES );
...
HRESULT CD3DSettingsDlg::OnWindowedFullScreenChanged()
{
    ...
    if( g_DeviceSettings.pp.Windowed )
    {
        ...
        if( pD3D == NULL )
            return DXTRACE_ERR( "DXUTGetD3DObject", E_FAIL );

        D3DDISPLAYMODE mode;
        hr = pD3D->GetAdapterDisplayMode( g_DeviceSettings.AdapterOrdinal, &mode );
        if( FAILED(hr) )
            return DXTRACE_ERR( "GetAdapterDisplayMode", hr );
...
HRESULT CD3DSettingsDlg::OnBackBufferFormatChanged()
{
    ...
    for( int idc=0; idc < pDeviceInfo->deviceSettingsComboList.GetSize(); idc++ )
    {
            ...
            pDeviceCombo->BackBufferFormat == backBufferFormat )
        {
            ...
            {
                if( !pDepthStencilComboBox->ContainsItem( "(not used)" ) )
                    pDepthStencilComboBox->AddItem( "(not used)", NULL );
            }
...
//-------------------------------------------------------------------------------------
void CD3DSettingsDlg::AddAdapter( const char* strDescription, UINT iAdapter )
{
...
void CD3DSettingsDlg::AddResolution( DWORD dwWidth, DWORD dwHeight )
{
    ...
    char strResolution[50];
    dwResolutionData = MAKELONG( dwWidth, dwHeight );
    StringCchPrintf( strResolution, 50, "%d by %d", dwWidth, dwHeight );
...
void CD3DSettingsDlg::AddRefreshRate( DWORD dwRate )
{
    ...
    char strRefreshRate[50];

    if( dwRate == 0 )
        StringCchCopy( strRefreshRate, 50, "Default Rate" );
    else
        StringCchPrintf( strRefreshRate, 50, "%d Hz", dwRate );
...
void CD3DSettingsDlg::AddMultisampleQuality( DWORD dwQuality )
{
    ...
    char strQuality[50];
    StringCchPrintf( strQuality, 50, "%d", dwQuality );
