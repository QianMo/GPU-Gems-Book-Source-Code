...
HRESULT CDXUTDialogResourceManager::OnD3D10ResizedSwapChain( ID3D10Device *pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    V_RETURN( DXUT_Dynamic_D3D10CreateStateBlock( m_pd3d10Device, &StateBlockMask, &m_pStateBlock10 ) );
...
void CDXUTComboBox::Render( float fElapsedTime )
{
    ...
    // NOTE: Fix by Andrew Lauritzen here...
    DXUTFontNode* pFont = m_pDialog->GetManager()->GetFontNode( pElement->iFont );
    if( pFont )
    {
