...
#endif
#ifndef SAFE_RELEASE_BASE_RESOURCE
#define SAFE_RELEASE_BASE_RESOURCE(p)  { if (p) { ID3D10Resource* __res; (p)->GetResource( &__res ); UINT ref = (__res)->Release(); if( ref ) { (__res)->Release(); } } }
#endif
...
LPCWSTR   WINAPI DXUTGetD3D10PipelineStats();
