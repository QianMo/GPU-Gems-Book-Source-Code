...
public:
    ...
    HRESULT DrawText10( LPCWSTR strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow = false, int nCount = -1 );
...
public:
    ...
    static  bool WINAPI StaticMsgProc( UINT uMsg, WPARAM wParam, LPARAM lParam );
...
protected:
    static WORD WINAPI GetLanguage() { return LOWORD( s_hklCurrent ); }
    static WORD WINAPI GetPrimaryLanguage() { return PRIMARYLANGID( LOWORD( s_hklCurrent ) ); }
    static WORD WINAPI GetSubLanguage() { return SUBLANGID( LOWORD( s_hklCurrent ) ); }
    ...
    static void WINAPI GetReadingWindowOrientation( DWORD dwId );
    static void WINAPI GetPrivateReadingString();
