...
float4 psReduceAddFirst(vertex2pixel v) : COLOR0 {
	...
	float2 coords2 = v.TexCoords.zy;
	float2 coords3 = v.TexCoords.xw;
	float2 coords4 = v.TexCoords.zw;
...
float4 psVectorMultiplyMatPosOnly0(vertex2pixel v) : COLOR0 {
	float2 f2ShiftCoords = computeShiftedCoordsPosOnly(v.TexCoords,fSize.z);
	return tex2D(sLastPass,v.TexCoords)+(tex2D(sVector,v.TexCoords)*tex2D(sVector2,f2ShiftCoords/fSize.xy)*fSize.w);
}

float4 psVectorMultiplyMatAllCases0(vertex2pixel v) : COLOR0 {
	float2 f2ShiftCoords = computeShiftedCoordsAllCases(v.TexCoords,fSize.z);
	return tex2D(sLastPass,v.TexCoords)+(tex2D(sVector,v.TexCoords)*tex2D(sVector2,f2ShiftCoords/fSize.xy)*fSize.w);
}
...
float4 psVectorMultiplyMatPosOnly1(vertex2pixel v) : COLOR0 {
	float4 f4ShiftCoords   = computeShiftedCoordsPosOnlyP1(v.TexCoords,fSize.z)/fSize.xyxy;

	float4 f4Result    = tex2D(sLastPass,v.TexCoords);
	float4 f4MatVector = tex2D(sVector,v.TexCoords)*fSize.w;
...
float4 psVectorMultiplyMatAllCases1(vertex2pixel v) : COLOR0 {
	float4 f4ShiftCoords   = computeShiftedCoordsAllCasesP1(v.TexCoords,fSize.z)/fSize.xyxy;
...
float4 psVectorMultiplyMatPosOnly0_noadd(vertex2pixel v) : COLOR0 {
	float2 f2ShiftCoords = computeShiftedCoordsPosOnly(v.TexCoords,fSize.z);
	return tex2D(sVector,v.TexCoords)*tex2D(sVector2,f2ShiftCoords/fSize.xy)*fSize.w;
}
