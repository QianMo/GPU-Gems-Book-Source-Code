...
float4 psDrop(vertex2pixel IN) : COLOR0 {
	return saturate(0.2-length(IN.TexCoords-f4Position.xy)*40)+tex2D(sHeightField,IN.TexCoords);
}
...
float4 psMain(vertex2pixelMain v) : COLOR0 {
	...
	// texture coordinates
	/*half2 uv = half2(v.TexCoords.x,1-v.TexCoords.y);
		
	half2 uvright      = saturate(half2(uv.x + f4StepSize.x, uv.y));
	half2 uvtop        = saturate(half2(uv.x, uv.y - f4StepSize.y));
	half2 uvleft       = saturate(half2(uv.x - f4StepSize.x, uv.y));
	half2 uvbottom     = saturate(half2(uv.x, uv.y + f4StepSize.y));
	
    // neighbour displacement values
    half4 f4Neigbours = half4(tex2D(sHeightField, uvright ).r,tex2D(sHeightField, uvtop ).r,tex2D(sHeightField, uvleft  ).r, tex2D(sHeightField, uvbottom).r);
    */
...
//	return half4(fHeight,-fHeight,0,1);
	...
	half4 normal4  = half4(normal.rgb,1);
...
float4 psMain(vertex2pixelMain v) : COLOR0 {
	...
	// texture coordinates
	/*float2 uv = float2(v.TexCoords.x,1-v.TexCoords.y);

	float2 uvright      = saturate(float2(uv.x + f4StepSize.x, uv.y));
	float2 uvtop        = saturate(float2(uv.x, uv.y - f4StepSize.y));
	float2 uvleft       = saturate(float2(uv.x - f4StepSize.x, uv.y));
	float2 uvbottom     = saturate(float2(uv.x, uv.y + f4StepSize.y));

    // neighbour displacement values
    float4 f4Neigbours = float4(tex2D(sHeightField, uvright ).r,tex2D(sHeightField, uvtop ).r,tex2D(sHeightField, uvleft  ).r, tex2D(sHeightField, uvbottom).r);
    */
...
//	return float4(fHeight,-fHeight,0,1);
	...
	float4 normal4  = float4(normal.rgb,1);
