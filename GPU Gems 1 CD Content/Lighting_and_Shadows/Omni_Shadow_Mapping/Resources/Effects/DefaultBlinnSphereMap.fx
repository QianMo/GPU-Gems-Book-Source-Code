...
texture tSphereReflectionMap;
PHG_FLOAT    fReflectionFactor;
...
PHG_FLOAT4 vUnpack = { 255.0, 1.0, 0.0, 0.0 };
			      ...
			      PHG_FLOAT4 vLight     : TEXCOORD5 ) : COLOR
{
	...
	PHG_FLOAT4 vSpecular = SelfShadow * vLightSpecular * pow( fNdotH, fMaterialPower ) * tex2D( SpecularMapSampler, vTexCoord );
...
// Pixel shader for last rendering pass

PHG_FLOAT4 BlinnPixelShaderLastPass ( PHG_FLOAT2 vTexCoord  : TEXCOORD0,
			              PHG_FLOAT3 vTangent   : TEXCOORD1,
			              PHG_FLOAT3 vBinormal  : TEXCOORD2,
			              PHG_FLOAT3 vNormal    : TEXCOORD3,
			              PHG_FLOAT3 vEye       : TEXCOORD4,
			              PHG_FLOAT4 vLight     : TEXCOORD5,
				      PHG_FLOAT2 vEnvMap    : TEXCOORD6 ) : COLOR
{
	// Light Direction ( xyz ) , Attenuaton Factor ( w )
...
#if !GREYSCALE
	return ( vDiffuse + vSpecular ) * vLightDirection.w * fShadow + tex2D( EnvironmentMapSampler, vEnvMap ) * fReflectionFactor;
#else
	return dot( ( vDiffuse + vSpecular ) * vLightDirection.w * fShadow + tex2D( EnvironmentMapSampler, vEnvMap ) * fReflectionFactor, vGreyScale );
#endif
...
// Vertex shader for last rendering pass

VS_OUTPUT_ENV BlinnVertexShaderLastPass( PHG_FLOAT4 vPosition : POSITION,
    			                 PHG_FLOAT4 vNormal   : NORMAL,
			                 PHG_FLOAT2 vTexCoord : TEXCOORD0,
			                 PHG_FLOAT3 vTangent  : TEXCOORD1,
			                 PHG_FLOAT3 vBinormal : TEXCOORD2 )
{
	...
	PHG_FLOAT3 vEyeNormal;
	vEyeNormal.xyz = mul( vNormal.xyz, mWV );
	vEyeNormal = normalize( vEyeNormal );
	
	o.vTexEnv.x = vEyeNormal.x * 0.5f + 0.5f;
	o.vTexEnv.y = -vEyeNormal.y * 0.5f + 0.5f;
...
{   
	pass p0
	{
		...
		PixelShader  = compile ps_2_0 BlinnPixelShaderLastPass();
		vertexshader = compile vs_2_0 BlinnVertexShaderLastPass();
	}
}
